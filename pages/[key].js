  ...
  return (
    ...
    >
      ...
      <div id="methodology" className="blog">
        ...
        <div
          className="block-paragraph"
          dangerouslySetInnerHTML={{ __html: texts.methodology }}
        ></div>
      </div>
      <ThemeNavigation
...
export async function getStaticProps(context) {
  ...
  const previous =
    currentIndex > 0
      ? structure.pages[currentIndex - 1]
      : structure.pages[structure.pages.length - 1];
  const next =
    currentIndex < structure.pages.length - 1
      ? structure.pages[currentIndex + 1]
      : structure.pages[0];
