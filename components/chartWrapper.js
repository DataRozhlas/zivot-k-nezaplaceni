...
export default function ChartWrapper({
  ...
  const legend = {
    items: dataProps.titles
      .map((t, i) => {
        return {
          color: dataProps.legendColors[i],
          title: t,
          description: legendDescriptions[i],
        };
      })
  ...
  const totalChart = (
    <div className="chart-content">
      <Chart
        key={`chart-${highlightedLineIndex ?? ""}`}
        lineStyles={dataProps.lineStyles}
        dataProps={getBigChartProps(dataProps, height, annotation, onHover)}
        chartType={chartType}
        filter={filter}
        highlightedLineIndex={highlightedLineIndex}
      />
